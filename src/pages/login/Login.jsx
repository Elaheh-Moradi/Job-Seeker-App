...
//          dispatch(authActions.loginSuccess(token)) // Save token to localStorage
...
import EnterJobSeeker from "./EnterJobSeeker";
import { LOGIN_TAB_1, LOGIN_TAB_2 } from "../../constants/names";
import RegisterJobSeeker from "./RegisterJobSeeker";
import { useLocation, useNavigate } from "react-router-dom";

const Login = ({ type,onLogin  }) => {
  const initialTab = type === "join" ? LOGIN_TAB_2 : LOGIN_TAB_1;
  const [activeTab, setActiveTab] = useState(initialTab);
  const navigate = useNavigate();
  const location = useLocation();

  useEffect(() => {
    if (location.pathname.includes("join/user")) {
      setActiveTab(LOGIN_TAB_2);
    } else {
      setActiveTab(LOGIN_TAB_1);
    }
  }, [location.pathname]);
  ...
  return (
    <div dir="rtl" className="bg-[#EEEEEE]">
      {activeTab === LOGIN_TAB_1 && (
        <label className="absolute text-[#999999] text-[20px] font-normal top-[17%] right-[35%] sm:top-[5%] sm:right-[10%]">
          ورود کارجو
        </label>
      )}
      {activeTab === LOGIN_TAB_2 && (
        <label className="absolute text-[#999999] text-[20px] font-normal top-[17%] right-[35%] sm:top-[5%] sm:right-[10%]">
          ایجاد حساب کاربری
        </label>
      )}

      <div className="h-screen flex items-start py-[12%] justify-center sm:py-[20%]">
        <div className=" w-[30%]  mx-auto  border-[1px] border-[#d0d0d0] sm:w-[80%] ">
          {/* Tab headers */}
          <div dir="rtl" className="flex items-center ">
            <button
              onClick={() => handleTabClick(LOGIN_TAB_1, "/login/user")}
              
              className={`py-2 px-4 flex-1 font-medium focus:outline-none ${
                activeTab === LOGIN_TAB_1
                  ? "border-t-2 border-[#3ab1e4] text-[#3ab1e4] bg-white"
                  : "font-extralight text-[#3ab1e4] border-b border-l border-gray-300 bg-[#f5f5f5]"
              }`}
            >
              ورود کارجو
            </button>
            <button
              onClick={() => handleTabClick(LOGIN_TAB_2, "/join/user")}
              className={`py-2 px-4 flex-1 font-medium focus:outline-none ${
                activeTab === LOGIN_TAB_2
                  ? "border-t-2 border-[#3ab1e4] text-[#3ab1e4] bg-white"
                  : "font-extralight text-[#3ab1e4] border-b border-r border-gray-300 bg-[#f5f5f5]"
              }`}
            >
              ثبت‌نام کارجو
            </button>
          ...
          <div className="p-4 bg-white">
            {activeTab === LOGIN_TAB_1 && (
              <div className="bg-white">
                <EnterJobSeeker onLogin={onLogin} />
              </div>
            )}
            {activeTab === LOGIN_TAB_2 && (
              <div className="bg-white">
